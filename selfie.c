...
void selfie_delete(int ID, int threadID);
...
int STACKSIZEPERTHREAD = 1048576; //  1MB of stack per thread = 2^20
...
int* get_shmo_frames(int *shmo);
int* get_shmo_clients(int *shmo);
...
int  getFrame(int *frameEntry);
int* getNextFrame(int *frameEntry);
void setNextFrame(int *frameEntry, int *next);
void setFrame(int *frameEntry, int *frame);
...
int* getNextClient(int *client);
int* getPrevClient(int *client);
int  getClientPID(int *client);
int* getClientPages(int *client);

void setNextClient(int *client, int *next);
...
void initInterpreter() {
  EXCEPTIONS = malloc(12 * SIZEOFINTSTAR);
...
int* findThread(int* threads, int id);
...
int* getPT(int* context)           { return (int*) *(context + 3); }
int  getBreak(int* context)        { return        *(context + 4); }
...
void implementShmSize() {
  ...
  // retrieve id and shSize from registers
  id = *(registers + REG_A0);
  shSize = *(registers + REG_A1);

  // search the shared memory object by the id
  shmo = find_shmo_by_id(id);

  // if the shared memory object does not exist, return -1 as size
  if (shmo == (int*) 0) {
    actSize = -1;
  }
  else {
    ...
    if (get_shmo_size(shmo) == 0) {
      // if current size is 0, replace it by shSize. Do nothing otherwise.
  ...
  // return the current size to the calling process
...
void implementShmOpen() {
  ...
  // extract name string (located in global segment of calling process)
  name = tlb(getPT(currentContext), *(registers + REG_A0));

  // look up the shared memory object by its name
...
int doThreadFork(int contextID, int threadID) {
  ...
  //copy all regs
  setThreadRegs(newThread, zalloc(NUMBEROFREGISTERS * WORDSIZE));
  i = 0;
  while (i < NUMBEROFREGISTERS) {
    *(getThreadRegs(newThread) + i) = *(getThreadRegs(fromThread) + i);
    i = i + 1;
  }
  ...
  if (debug_threadFork) {
    ...
    print((int*) " FP ");
    printInteger(*(getThreadRegs(fromThread) + REG_FP));
    println();
  }
  //copy top down
  while (vAddressFromThread >= SP_from - 13*WORDSIZE) { //max 15 arguments in the current method
  ...
  if(*(getThreadRegs(fromThread) + REG_FP) + 2 * WORDSIZE > vAddressFromThread - 13 * WORDSIZE){
  ...
  //SP_from = *(tlb(getPT(toContext), SP_from)) - threadsSP_diff;
  ...
  SP_from = *(getThreadRegs(fromThread) + REG_SP) - threadsSP_diff;

  //set new SP pointer
  //*(getThreadRegs(newThread) + REG_SP) = *(tlb(getPT(toContext), SP_from)) - threadsSP_diff - 6 * WORDSIZE;
  *(getThreadRegs(newThread) + REG_SP) = SP_from;//- 2 * WORDSIZE;
  if (*(getThreadRegs(fromThread) + REG_FP) >= vAddressFromThread - 13 * WORDSIZE){
    *(getThreadRegs(newThread) + REG_SP) = *(getThreadRegs(fromThread) + REG_SP) - threadsSP_diff ;//+ 2 * WORDSIZE;
...
int doSwitch(int toID, int toThreadID) {
  ...
  if (toContext != (int*) 0) {
    ...
    if (debug_switch) {
      ...
      printInteger(toID);
      print((int*) " thread ");
      printInteger(getThreadID(getCurrentThread(toContext)));
      println();
    }
...
int hypster_switch(int toID, int threadID) {
  // this procedure is only executed at boot level zero
...
void emitDelete() {
  ...
  emitIFormat(OP_LW, REG_SP, REG_A1, 0); // thread ID
  emitIFormat(OP_ADDIU, REG_SP, REG_SP, WORDSIZE);
...
void implementDelete() {
  doDelete(*(registers+REG_A0), *(registers+REG_A1));
}

void hypster_delete(int ID, int threadID) {
  // this procedure is only executed at boot level zero
  doDelete(ID, threadID);
}
...
int* get_shmo_frames(int *shmo)                { return (int*) *(shmo + 5); }
int* get_shmo_clients(int *shmo)               { return (int*) *(shmo + 6); }
...
int getFrame(int *frameEntry)      { return *frameEntry; }
int* getNextFrame(int *frameEntry) { return (int*) *(frameEntry + 1); }

void setFrame(int *frameEntry, int *frame)    {*frameEntry = (int) frame; }
void setNextFrame(int *frameEntry, int *next) {*(frameEntry + 1) = (int) next; }
...
int  getClientPID(int *client)   { return *(client + 2); }
int* getClientPages(int *client) { return (int*) *(client + 3); }

void setNextClient(int *client, int *next)   { *client = (int) next; }
...
int* allocateContext(int ID, int parentID) {
  ...
  if (freeContexts == (int*) 0)
    context = malloc(5 * SIZEOFINTSTAR + 3 * SIZEOFINT);
  else {
